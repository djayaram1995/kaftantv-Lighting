export default class Details extends lng.Component {
    ...
    return {
      ...
      Details: {
        ...
        Metadata: {
          ...
          Title: {
            w: 900,
            text: { fontFace: "Roboto-Regular", fontSize: 51, lineHeight: 50 }
          },
          Year: {
            w: 900,
            text: { fontFace: "Roboto-Regular", fontSize: 28, lineHeight: 50 }
          },
          Info: {
            w: 700,
            text: { fontFace: "-Regular", fontSize: 39, lineHeight: 60 }
          }
        }
      }
    };
  ...
  _register() {
    ...
    this.tag("Blur")
      .transition("amount")
      .on("finish", this._events.showDetails);
  }
  ...
  _updateDetails(item) {
    if (item.title != undefined) {
      this.patch({
        Details: {
          Poster: {
            src: item._links.awsThumbnailUrl.href
          },
          Metadata: {
            Title: { text: { text: item.title } },
            Year: { text: { text: item.year } },
            Info: { text: { text: item.description } }
          }
        }
      });
    } else {
